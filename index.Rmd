...
```{r, echo=FALSE}
inputPanel(
  fileInput("fasta","Choose sequence file"),
...
seqs <- reactive({
    ...
    read.dna(input$fasta$datapath[1],format="fasta",as.matrix=TRUE)
})
```
...
```{r, echo=FALSE}
actionButton("drawalign","Plot alignment")
renderPlot({
  if(input$drawalign==0) return()
  image(seqs())
})
```

## Getting a rough tree

```{r,echo=FALSE}
actionButton("calcdist","Calculate distances")
dst <- reactive({
  if(input$calcdist==0) return()
  dist.dna(seqs(),model="TN93",as.matrix=TRUE)
})
```

```{r, echo=FALSE}
actionButton("drawdist","Plot distances")
renderPlot({
  if(input$drawdist==0) return()
  d <- dst()
  d <- d[lower.tri(d)]
  hist(d)
})
...
```{r,echo=FALSE}
actionButton("calcnj","NJ tree")
njt <- reactive({
  if(input$calcnj==0) return()
  nj(dst())
})
```

```{r, echo=FALSE}
actionButton("drawnj","Plot NJ tree")
renderPlot({
  if(input$drawnj==0) return()
  plot(njt(),type="unrooted")
})
```
...
```{r, echo=FALSE}
actionButton("mtest","Model test")
mt <- reactive({
  if(input$mtest==0) return()
  modelTest(as.phyDat(seqs()),model=c("JC","HKY","TN93","GTR"))
})
renderDataTable(mt())
```
...
```{r, echo=FALSE}
actionButton("initlike","Initialise likelihood")
mlobj <- reactive({
  if(input$initlike==0) return()
  p <- pml(as.phylo(njt()),as.phyDat(seqs()))
  p
})
renderPrint(print(mlobj()))
```

### Optimising the likelihood

```{r, echo=FALSE}
actionButton("optlike","Optimise likelihood")
optmlobj <- reactive({
  if(input$optlike==0) return()
  op <- optim.pml(mlobj(),optNni=TRUE,optBf=TRUE,optQ=TRUE,optGamma=TRUE,optInv=TRUE,optEdge=TRUE,model="GTR")
})
renderPrint(print(optmlobj()))
```
...
treertt <- reactive({
  if(input$rtt==0) return()
    tr <- optmlobj()$tree
    tr <- multi2di(tr)
    tr$edge.length <- tr$edge.length+1e-6
    td <- tr$tip.label %>% strsplit(.,"_") %>% lapply(.,tail,1) %>% unlist %>% as.double
    r <- rtt(tr,td,objective="rsquared")
    r
})
renderPlot({
  tr.rtt <- treertt()
